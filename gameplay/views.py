...
from gameplay.models import Users, Cards, Words, Games, Rounds, Prises, Scores, UsersPrises
...
def signup_view(request: HttpRequest):
    if request.method == 'POST':
        ...
        form = SignUpForm()
    return render(request, 'gameplay/signup.html', {'form': form, 'user': None})
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if user is not None:
                ...
                return redirect('home')  # Перенаправляем на главную страницу
    return render(request, 'gameplay/login.html', {'form': form, 'user': None})
...
def game_score_view(request: HttpRequest):
    ...
    return render(request, 'gameplay/game_score.html', context)
...
def game_prise_view(request: HttpRequest):
    ...
    return render(request, 'gameplay/game_prise.html', context)
        ...
        if user.user_id != game.user_id:
            ...
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    return render(request, 'gameplay/game_guess.html', context)
                request.session['drum_score'] = None
                return redirect('game_score')
            else:
                form = GameForm()
...
def finish_view(request: HttpRequest):
    ...
    return render(request, 'gameplay/finish.html', context)
...
def rules_view(request):
    ...
    return render(request, 'gameplay/rules.html', context)
...
def exchange_view(request: HttpRequest):
    ...
    return render(request, 'gameplay/exchange.html', context)
